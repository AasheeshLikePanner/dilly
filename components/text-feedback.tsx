...
import { Send, X, MessageSquare, Check, Sparkles } from 'lucide-react';
...
export default function TextInputFeedback({ apiKey, showcaseMode = false, onSuccess, onError }: TextFeedbackProps = {}) {
  ...
  return (
    <motion.div
      className="relative w-full max-w-[500px] bg-[#121214] border border-white/5 rounded-[28px] overflow-hidden z-10 ring-1 ring-white/5"
    >
      {/* Success Overlay */}
      <AnimatePresence>
        {status === 'sent' && (
          ...
          >
            ...
            <div className="relative w-4 h-4">
              ...
              {status === 'sending' && (
                <motion.div
                  key="icon-flying"
                  className="absolute inset-0 text-black" // Plane matches text color
            ...
            {/* Trail Effect (Subtle wind lines) */}
            {status === 'sending' && (
              <motion.div
